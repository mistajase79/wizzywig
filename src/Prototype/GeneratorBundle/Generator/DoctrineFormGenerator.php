...
class DoctrineFormGenerator extends Generator
{
     ...
     */
    public function generate(BundleInterface $bundle, $entity, ClassMetadataInfo $metadata, $forceOverwrite = false, $makeTranslatable = false)
    {
        ...
        if($makeTranslatable){
            $this->className = $entityClass.'TranslationType';
            $this->classPath = $dirPath.'/'.str_replace('\\', '/', $entity).'TranslationType.php';

            $this->renderFile('form/TranslationFormType.php.twig', $this->classPath, array(
                'fields' => $this->getFieldsFromMetadata($metadata),
                'fields_mapping' => $metadata->fieldMappings,
                'namespace' => $bundle->getNamespace(),
                'entity_namespace' => implode('\\', $parts),
