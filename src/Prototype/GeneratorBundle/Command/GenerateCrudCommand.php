...
use Prototype\GeneratorBundle\Generator\DoctrineCrudGenerator;
use Prototype\GeneratorBundle\Generator\DoctrineCrudTranslationGenerator;

#use Sensio\Bundle\GeneratorBundle\Generator\DoctrineFormGenerator;
use Prototype\GeneratorBundle\Generator\DoctrineFormGenerator;
 ...
 */
class GenerateCrudCommand extends \Prototype\GeneratorBundle\Command\GenerateDoctrineCrudCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setDefinition(array(
                ...
                new InputOption('translate', '', InputOption::VALUE_NONE, 'Make translational '),
            ))
            ->setHelp(<<<EOT
*** CUSTOM PROTOTYPE CRUD GENERATOR ***

A CUSTOM PATH HAS BEEN ADDED FOR PROTOCMSv2
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $entity = \Sensio\Bundle\GeneratorBundle\Command\Validators::validateEntityName($input->getOption('entity'));
        list($bundle, $entity) = $this->parseShortcutNotation($entity);

        $format = \Sensio\Bundle\GeneratorBundle\Command\Validators::validateFormat($input->getOption('format'));
        $prefix = $this->getRoutePrefix($input, $entity);
        $withWrite = $input->getOption('with-write');
        $forceOverwrite = $input->getOption('overwrite');
        $makeTranslatable = $input->getOption('translate');
        ...
        $generator->generate($bundle, $entity, $metadata[0], $format, $prefix, $withWrite, $forceOverwrite, $makeTranslatable);
        ...
        // form
        if ($withWrite) {
            $this->generateForm($bundle, $entity, $metadata, $forceOverwrite, $makeTranslatable);
        ...
        if ('annotation' != $format) {
            $runner($this->updateRouting($questionHelper, $input, $output, $bundle, $format, $entity, $prefix, $makeTranslatable));
        } else {
    ...
    {
        $questionHelper = $this->getQuestionHelper();
        $questionHelper->writeSection($output, 'Welcome to the PROTOCMSv2 Doctrine2 CRUD generator');
        ...
        // translatable?
        $translate = $input->getOption('translate') ?: true;
        $output->writeln(array(
            '',
            'Do you wish to make this translatable',
            '',
        ));
        $question = new ConfirmationQuestion($questionHelper->getQuestion('Do you want to generate "translatable" actions (formtype, controller)', $translate ? 'yes' : 'no', '?', $translate), $translate);

        $translate = $questionHelper->ask($input, $output, $question);
        $input->setOption('translate', $translate);
     ...
     */
    protected function generateForm($bundle, $entity, $metadata, $forceOverwrite = false, $makeTranslatable = false)
    {
        $this->getFormGenerator($bundle)->generate($bundle, $entity, $metadata[0], $forceOverwrite, $makeTranslatable);
    }

    protected function updateRouting(QuestionHelper $questionHelper, InputInterface $input, OutputInterface $output, BundleInterface $bundle, $format, $entity, $prefix, $makeTranslatable = false)
    {
        $auto = true;
        if ($input->isInteractive()) {
    ...
    protected function createTranslationGenerator($bundle = null)
    {
	    return new DoctrineCrudTranslationGenerator(
            $this->getContainer()->get('filesystem'),
            $this->getContainer()->getParameter('kernel.root_dir')
        );
    }
